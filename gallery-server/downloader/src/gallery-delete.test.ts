...
import { deleteGalleries } from './gallery-delete.js';

test('provider-local deletion removes loose data and Hitomi archive entries', () => {
    ...
    try {
        assert.deepEqual(deleteGalleries('hitomi', [123], { workingDir }), {
            deleted: [123],
            skipped: [],
        });
...
test('active gallery deletion is skipped without touching its files', () => {
    ...
    try {
        assert.deepEqual(deleteGalleries('imhentai', [456], { workingDir }), {
            deleted: [],
